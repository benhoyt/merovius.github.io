...
for generics in Go have dropped. Since then, predictably, there has been a
bunch of chatter on the intertubez about it. This is a summary of my thoughts,
so far, on the subject - after a bunch of discussions on Twitter and Reddit.

#### Contracts vs. Interfaces

First, there is a common observation of overlap between Contracts and Generics.
To untangle that, we can say that when we say "generics", what we mean is
*constrained parametric polymorphism*. Go already allows polymorphism by using
interfaces. Contracts add two things: One, a way to add *type-parameters* to
functions and types. And two, a syntax to constrain those type-parameters to a
subset that allows specific operations.
...
contract) containing all the statements/expressions you would wish to be able
...
type-variable in the contract body and allow the instantiation, if it
type-checks.
...
simple example is map-keys. The draft design uses the contract
...
If generic code is compiled generically, it still needs to employ some method
...
I've previously said that I'm "meh" on the contracts proposal, which is the
strongest form of endorsement a generics proposal could ever get from me. After
some discussion, I'm more and more convinced that while contracts *seem*
...
derived constraints or writing contracts. There are also a bunch of open
questions in regards to contracts, a bunch of them related to the ambiguity
of Go-expressions. As a result, I'm starting to feel more negative towards them
...
to be taken from the Contracts design with minimal changes. It's entirely
